...
static int stm32_lptim_func_map(enum stm32_lptim_cnt_function *const cnt_func,
				const enum counter_function function)
{
	switch (function) {
	case COUNTER_FUNCTION_INCREASE:
		*cnt_func = STM32_LPTIM_COUNTER_INCREASE;
		return 0;
	case COUNTER_FUNCTION_QUADRATURE_X4:
		*cnt_func = STM32_LPTIM_ENCODER_BOTH_EDGE;
		return 0;
	default:
		return -EINVAL;
	}
...
{
	struct stm32_lptim_cnt *const priv = counter->priv;
	enum stm32_lptim_cnt_function cnt_func;
	int err;

	if (stm32_lptim_is_enabled(priv))
		return -EBUSY;

	err = stm32_lptim_func_map(&cnt_func, function);
	if (err)
		return err;
					...
					enum counter_synapse_action action)
{
	...
	if (cnt_func != STM32_LPTIM_COUNTER_INCREASE
