...
{
	struct drm_i915_private *dev_priv = m->private;
	struct i915_hotplug *hotplug = &dev_priv->display->hotplug;

	/* Synchronize with everything first in case there's been an HPD
	 * storm, but we haven't finished handling it in the kernel yet
	 */
	intel_synchronize_irq(dev_priv);
	flush_work(&dev_priv->_display.hotplug.dig_port_work);
	flush_delayed_work(&dev_priv->_display.hotplug.hotplug_work);
...
static int i915_hpd_short_storm_ctl_show(struct seq_file *m, void *data)
{
	...
	seq_printf(m, "Enabled: %s\n",
		   yesno(dev_priv->display->hotplug.hpd_short_storm_enabled));
