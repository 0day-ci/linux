 ...
 */
static inline void *txq_end(const struct funeth_txq *q)
{
	return (void *)q->hw_wb;
}
 ...
 * end.
 */
static inline unsigned int txq_to_end(const struct funeth_txq *q, void *p)
{
	return txq_end(q) - p;
}

/* Return the number of Tx descriptors occupied by a Tx request. */
static inline unsigned int tx_req_ndesc(const struct fun_eth_tx_req *req)
{
	return DIV_ROUND_UP(req->len8, FUNETH_SQE_SIZE / 8);
}

static inline __be16 tcp_hdr_doff_flags(const struct tcphdr *th)
{
 ...
 * descriptor unused.
 */
static inline unsigned int fun_txq_avail(const struct funeth_txq *q)
{
...
static inline u16 txq_hw_head(const struct funeth_txq *q)
{
