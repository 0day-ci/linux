...
int fun_txq_create_dev(struct funeth_txq *q, struct fun_irq *irq)
{
	...
	*q->hw_wb = 0;
	...
	if (irq) {
		...
		q->ethid += q->netdev->real_num_tx_queues;
	}
	...
	netif_info(fp, ifup, q->netdev,
		   ...
		   irq ? "Tx" : "XDP", q->qidx, ndesc, q->hw_qid, q->irq_idx,
		   q->ethid, q->numa_node);
	return 0;
...
static void fun_txq_free_dev(struct funeth_txq *q)
{
	...
	netif_info(fp, ifdown, q->netdev,
		   ...
		   q->irq_idx ? "Tx" : "XDP", q->qidx, q->hw_qid,
		   q->irq_idx, q->ethid);
	fun_destroy_sq(fp->fdev, q->hw_qid);
...
/* Create a Tx queue, allocating all the host and device resources needed to
