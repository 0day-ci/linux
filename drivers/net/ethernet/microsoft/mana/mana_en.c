...
static void mana_cleanup_context(struct mana_port_context *apc)
{
	struct gdma_dev *gd = apc->ac->gdma_dev;
...
static int mana_init_context(struct mana_port_context *apc)
{
	struct gdma_dev *gd = apc->ac->gdma_dev;
	int err;
			     ...
			     u32 in_buf_len, void *out_buf, u32 out_buf_len)
{
	...
	struct gdma_send_mana_message_resp *resp = NULL;
	struct gdma_send_mana_message_req *req = NULL;
	struct device *dev = gc->dev;
	int err;

	if (mana_is_gdma_msg_len(in_buf_len, out_buf_len, in_buf)) {
		struct gdma_req_hdr *g_req = in_buf;
		struct gdma_resp_hdr *g_resp = out_buf;

		static atomic_t act_id;

		g_req->dev_id = gc->mana.dev_id;
		...
		if (err || resp->hdr.status) {
			dev_err(dev, "Send MANA message failed: %d, 0x%x\n",
				err, resp->hdr.status);
			if (!err)
				err = -EPROTO;
			goto out;
...
static void mana_key_table_init(struct mana_port_context *apc, bool reset_hash)
{
	int i;
