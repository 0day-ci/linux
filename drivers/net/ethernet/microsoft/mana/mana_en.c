...
static int mana_map_skb(struct sk_buff *skb, struct ana_port_context *apc,
			     ...
			     int old_q)
{
	...
	txq = apc->indir_table[hash & ANA_INDIRECT_TABLE_MASK];
...
static void mana_cleanup_context(struct ana_port_context *apc)
{
	struct gdma_dev *gd = apc->ac->gdma_dev;
...
static int mana_init_context(struct ana_port_context *apc)
{
	struct gdma_dev *gd = apc->ac->gdma_dev;
	int err;
	...
	apc->rxqs = kcalloc(apc->num_queues, sizeof(struct ana_rxq *),
			    GFP_KERNEL);
	if (!apc->rxqs)
		return -ENOMEM;
...
static int mana_send_request(struct ana_context *ac, void *in_buf,
			     u32 in_buf_len, void *out_buf, u32 out_buf_len)
{
	...
	if (is_gdma_msg_len(in_buf_len, out_buf_len, in_buf)) {
		...
		err = mana_gd_send_request(gc, in_buf_len, in_buf, out_buf_len,
					   out_buf);
		if (err || g_resp->status) {
			dev_err(dev, "Send GDMA message failed: %d, 0x%x\n",
				err, g_resp->status);
			return -EPROTO;
		}
		...
		if (g_req->dev_id.as_uint32 != g_resp->dev_id.as_uint32 ||
			...
			dev_err(dev, "Wrong GDMA response: %x,%x,%x,%x\n",
		...
		err = mana_gd_send_request(gc, req_size, req, resp_size, resp);
		if (err || resp->hdr.status) {
			dev_err(dev, "Send ANA message failed: %d, 0x%x\n",
				err, resp->hdr.status);
			if (!err)
				err = -EPROTO;
			goto out;
				 ...
				 u16 *max_num_vports)
{
	...
	err = mana_verify_resp_hdr(&resp.hdr, ANA_QUERY_CLIENT_CONFIG,
				   sizeof(resp));
	if (err || resp.hdr.status) {
...
static int mana_query_vport_cfg(struct ana_port_context *apc, u32 vport_index,
				u32 *max_sq, u32 *max_rq, u32 *num_indir_entry)
{
	...
	mana_gd_init_req_hdr(&req.hdr, ANA_QUERY_VPORT_CONFIG,
			     sizeof(req), sizeof(resp));
	...
	err = mana_verify_resp_hdr(&resp.hdr, ANA_QUERY_VPORT_CONFIG,
				   sizeof(resp));
	if (err)
		return err;
...
static int mana_cfg_vport(struct ana_port_context *apc, u32 protection_dom_id,
			  u32 doorbell_pg_id)
{
	...
	mana_gd_init_req_hdr(&req.hdr, ANA_CONFIG_VPORT_TX,
			     sizeof(req), sizeof(resp));
	...
	err = mana_verify_resp_hdr(&resp.hdr, ANA_CONFIG_VPORT_TX,
				   sizeof(resp));
	if (err || resp.hdr.status) {
...
static int mana_cfg_vport_steering(struct ana_port_context *apc,
				   enum TRI_STATE rx,
				   ...
				   bool update_tab)
{
	...
	mana_gd_init_req_hdr(&req->hdr, ANA_CONFIG_VPORT_RX, req_buf_size,
			     sizeof(resp));
	...
	if (update_key)
		memcpy(&req->hashkey, apc->hashkey, ANA_HASH_KEY_SIZE);
	...
	err = mana_verify_resp_hdr(&resp.hdr, ANA_CONFIG_VPORT_RX,
				   sizeof(resp));
	if (err) {
...
static int mana_create_wq_obj(struct ana_port_context *apc, ana_handle_t vport,
			      ...
			      ana_handle_t *wq_obj)
{
	...
	mana_gd_init_req_hdr(&req.hdr, ANA_CREATE_WQ_OBJ,
			     sizeof(req), sizeof(resp));
	req.vport = vport;
	req.wq_type = wq_type;
	...
	err = mana_verify_resp_hdr(&resp.hdr, ANA_CREATE_WQ_OBJ,
				   sizeof(resp));
	if (err || resp.hdr.status) {
...
static void mana_destroy_wq_obj(struct ana_port_context *apc, u32 wq_type,
				ana_handle_t wq_obj)
{
	...
	mana_gd_init_req_hdr(&req.hdr, ANA_DESTROY_WQ_OBJ,
			     sizeof(req), sizeof(resp));
	req.wq_type = wq_type;
	...
	err = mana_verify_resp_hdr(&resp.hdr, ANA_DESTROY_WQ_OBJ,
				   sizeof(resp));
	if (err || resp.hdr.status)
...
static int mana_create_eq(struct ana_port_context *apc)
{
	struct gdma_dev *gd = apc->ac->gdma_dev;
	...
	apc->eqs = kcalloc(apc->num_queues, sizeof(struct ana_eq),
			   GFP_KERNEL);
	if (!apc->eqs)
		return -ENOMEM;
	...
	for (i = 0; i < apc->num_queues; i++) {
		...
		err = mana_gd_create_ana_eq(gd, &spec, &apc->eqs[i].eq);
		if (err)
			goto out;
	}
...
static void mana_unmap_skb(struct sk_buff *skb, struct ana_port_context *apc)
{
	struct ana_skb_head *ash = (struct ana_skb_head *)skb->head;
...
static void mana_poll_tx_cq(struct ana_cq *cq)
{
...
static void mana_post_pkt_rxq(struct ana_rxq *rxq)
{
...
static void mana_rx_skb(void *buf_va, struct ana_rxcomp_oob *cqe,
			struct ana_rxq *rxq)
{
...
static void mana_process_rx_cqe(struct ana_rxq *rxq, struct ana_cq *cq,
...
static void mana_poll_rx_cq(struct ana_cq *cq)
{
...
static void mana_deinit_cq(struct ana_port_context *apc, struct ana_cq *cq)
{
	struct gdma_dev *gd = apc->ac->gdma_dev;
...
static void mana_deinit_txq(struct ana_port_context *apc, struct ana_txq *txq)
{
	struct gdma_dev *gd = apc->ac->gdma_dev;
...
static void mana_destroy_txq(struct ana_port_context *apc)
{
	int i;
...
static int mana_create_txq(struct ana_port_context *apc, struct net_device *net)
{
	struct gdma_dev *gd = apc->ac->gdma_dev;
	...
	apc->tx_qp = kcalloc(apc->num_queues, sizeof(struct ana_tx_qp),
			     GFP_KERNEL);
	if (!apc->tx_qp)
		return -ENOMEM;
	...
	for (i = 0; i < apc->num_queues; i++) {
		apc->tx_qp[i].tx_object = INVALID_ANA_HANDLE;
		...
		err = mana_gd_create_ana_wq_cq(gd, &spec, &txq->gdma_sq);
		if (err)
			goto out;
		...
		spec.cq.context = cq;
		err = mana_gd_create_ana_wq_cq(gd, &spec, &cq->gdma_cq);
		if (err)
			goto out;
...
static void mana_napi_sync_for_rx(struct ana_rxq *rxq)
{
	struct net_device *ndev = rxq->ndev;
	struct ana_port_context *apc;
	u16 rxq_idx = rxq->rxq_idx;
...
static void mana_destroy_rxq(struct ana_port_context *apc, struct ana_rxq *rxq,
			     bool validate_state)
...
static int mana_alloc_rx_wqe(struct ana_port_context *apc, struct ana_rxq *rxq,
...
static int mana_push_wqe(struct ana_rxq *rxq)
{
	struct ana_recv_buf_oob *rx_oob;
	u32 buf_idx;
	int err;
...
static struct ana_rxq *mana_create_rxq(struct ana_port_context *apc,
				       u32 rxq_idx, struct ana_eq *eq,
				       struct net_device *ndev)
{
	struct gdma_dev *gd = apc->ac->gdma_dev;
	struct ana_obj_spec wq_spec;
	...
	err = mana_gd_create_ana_wq_cq(gd, &spec, &rxq->gdma_rq);
	if (err)
		goto out;
...
static int mana_add_rx_queues(struct ana_port_context *apc,
			      struct net_device *ndev)
{
	struct ana_rxq *rxq;
	int err = 0;
...
static void mana_destroy_vport(struct ana_port_context *apc)
{
	struct ana_rxq *rxq;
	u32 rxq_idx;
...
static int mana_create_vport(struct ana_port_context *apc,
			     struct net_device *net)
{
	struct gdma_dev *gd = apc->ac->gdma_dev;
	int err;

	apc->default_rxobj = INVALID_ANA_HANDLE;

	err = mana_cfg_vport(apc, gd->pdid, gd->doorbell);
	if (err)
		return err;

	err = mana_create_txq(apc, net);
	return err;
}

static void mana_key_table_init(struct ana_port_context *apc, bool reset_hash)
{
	int i;

	if (reset_hash)
		get_random_bytes(apc->hashkey, ANA_HASH_KEY_SIZE);

	for (i = 0; i < ANA_INDIRECT_TABLE_SIZE; i++)
		apc->indir_table[i] = i % apc->num_queues;
}

int mana_config_rss(struct ana_port_context *apc, enum TRI_STATE rx,
...
static int mana_do_attach(struct net_device *ndev, int port_idx,
			  bool reset_hash)
{
	struct ana_port_context *apc = netdev_priv(ndev);
	struct gdma_dev *gd = apc->ac->gdma_dev;
	...
	mana_key_table_init(apc, reset_hash);
...
int mana_attach(struct net_device *ndev)
{
	...
	err = mana_do_attach(ndev, apc->port_idx, false);
	if (err)
		return err;
...
int mana_detach(struct net_device *ndev)
{
	struct ana_port_context *apc = netdev_priv(ndev);
	struct ana_txq *txq;
	int i, err;
...
static int mana_probe_port(struct ana_context *ac, int port_idx,
			   struct net_device **ndev_storage)
{
	...
	ndev = alloc_etherdev_mq(sizeof(struct ana_port_context),
				 gc->max_num_queues);
	if (!ndev)
		return -ENOMEM;
	...
	*ndev_storage = ndev;
	...
	apc->num_queues = min_t(uint, gc->max_num_queues,
				ANA_DEFAULT_NUM_QUEUES);
	apc->port_handle = INVALID_ANA_HANDLE;
	...
	netif_carrier_off(ndev);
	err = mana_do_attach(ndev, port_idx, true);
	if (err)
		goto free_net;
...
int mana_probe(struct gdma_dev *gd)
{
	...
	dev_info(dev,
		 "Microsoft Azure Network Adapter driver version: %d.%d.%d\n",
	...
	err = mana_query_client_cfg(ac, ANA_MAJOR_VERSION, ANA_MINOR_VERSION,
				    ANA_MICRO_VERSION, &ac->num_ports);
	if (err)
		goto out;

	if (ac->num_ports > MAX_PORTS_IN_ANA_DEV)
		ac->num_ports = MAX_PORTS_IN_ANA_DEV;
...
{
	struct gdma_context *gc = gd->gdma_context;
	struct ana_context *ac = gd->driver_data;
	struct device *dev = gc->dev;
	struct net_device *ndev;
	int i;
